...
def main(args):
    ...
    for p in tqdm(patients) if verbose else patients:
        try:
            # Read images:
            img_ct = read_nifti(path_to_input / (p + '_ct.nii.gz'))
            img_pt = read_nifti(path_to_input / (p + '_pt.nii.gz'))
            if is_mask_available:
                mask = read_nifti(path_to_input / (p + '_gtvt.nii.gz'))
            ...
            if is_mask_available:
                write_nifti(cr_mask, path_to_output / p / (p + '_gtvt.nii.gz'))
        
        except:
            print("{} not exist".format(p))
            pass
